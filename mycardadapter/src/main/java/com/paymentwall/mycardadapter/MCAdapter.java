...
public class MCAdapter {
    ...
    public MCAdapter(Fragment localPsFragment) {
        try {
    ...
    private void pay(final Context context, Parcelable params, Map<String, String> bundle, Map<String, String> customParams) {
        ...
        Bundle b = new Bundle();
        b.putParcelable(MCFragment.MYCARD_OBJECT, psMyCard);
        try {
            mthReplaceFragment.invoke(psActivity, MCFragment.getInstance(), b);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
