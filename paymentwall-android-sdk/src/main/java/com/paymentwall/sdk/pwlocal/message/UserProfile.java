...
public class UserProfile implements Parameterable, Parcelable {
    ...
    public Map<String, String> toParameters() {
        ...
        if(!TextUtils.isEmpty(email)) parameters.put(Parameters.EMAIL, MiscUtils.val(email));
        if(registrationDate!=null) parameters.put(Parameters.REGISTRATION_DATE, MiscUtils.val(registrationDate));
        if(birthday!=null) parameters.put(Parameters.BIRTHDAY, MiscUtils.val(birthday));
        if(!TextUtils.isEmpty(sex)) parameters.put(Parameters.SEX, MiscUtils.val(sex));
        if(!TextUtils.isEmpty(username)) parameters.put(Parameters.USERNAME, MiscUtils.val(username));
        if(!TextUtils.isEmpty(firstname)) parameters.put(Parameters.FIRSTNAME, MiscUtils.val(firstname));
        if(!TextUtils.isEmpty(lastname)) parameters.put(Parameters.LASTNAME, MiscUtils.val(lastname));
        if(!TextUtils.isEmpty(city)) parameters.put(Parameters.CITY, MiscUtils.val(city));
        if(!TextUtils.isEmpty(state)) parameters.put(Parameters.STATE, MiscUtils.val(state));
        if(!TextUtils.isEmpty(address)) parameters.put(Parameters.ADDRESS, MiscUtils.val(address));
        if(!TextUtils.isEmpty(country)) parameters.put(Parameters.COUNTRY, MiscUtils.val(country));
        if(!TextUtils.isEmpty(zip)) parameters.put(Parameters.ZIP, MiscUtils.val(zip));
        if(!TextUtils.isEmpty(membership)) parameters.put(Parameters.MEMBERSHIP, MiscUtils.val(membership));
        if(membershipDate!=null) parameters.put(Parameters.MEMBERSHIP_DATE, MiscUtils.val(membershipDate));
        if(!TextUtils.isEmpty(registrationCountry)) parameters.put(Parameters.REGISTRATION_COUNTRY, MiscUtils.val(registrationCountry));
        if(!TextUtils.isEmpty(registrationIp)) parameters.put(Parameters.REGISTRATION_IP, MiscUtils.val(registrationIp));
        if(!TextUtils.isEmpty(registrationEmail)) parameters.put(Parameters.REGISTRATION_EMAIL, MiscUtils.val(registrationEmail));
        if(registrationEmailVerified!=null) parameters.put(Parameters.REGISTRATION_EMAIL_VERIFIED, MiscUtils.val(registrationEmailVerified));
        if(!TextUtils.isEmpty(registrationName)) parameters.put(Parameters.REGISTRATION_NAME, MiscUtils.val(registrationName));
        if(!TextUtils.isEmpty(registrationLastname)) parameters.put(Parameters.REGISTRATION_LASTNAME, MiscUtils.val(registrationLastname));
        if(!TextUtils.isEmpty(registrationSource)) parameters.put(Parameters.REGISTRATION_SOURCE, MiscUtils.val(registrationSource));
        if(loginsNumber!=null) parameters.put(Parameters.LOGINS_NUMBER, MiscUtils.val(loginsNumber));
        if(paymentsNumber!=null) parameters.put(Parameters.PAYMENTS_NUMBER, MiscUtils.val(paymentsNumber));
        if(paymentsAmount!=null) parameters.put(Parameters.PAYMENTS_AMOUNT, MiscUtils.val(paymentsAmount));
        if(followers!=null) parameters.put(Parameters.FOLLOWERS, MiscUtils.val(followers));
        if(messageSent!=null) parameters.put(Parameters.MESSAGE_SENT, MiscUtils.val(messageSent));
        if(messageSentLast24h!=null) parameters.put(Parameters.MESSAGE_SENT_LAST_24H, MiscUtils.val(messageSentLast24h));
        if(messageReceived!=null) parameters.put(Parameters.MESSAGE_RECEIVED, MiscUtils.val(messageReceived));
        if(interactions!=null) parameters.put(Parameters.INTERACTIONS, MiscUtils.val(interactions));
        if(interactionsLast24h!=null) parameters.put(Parameters.INTERACTIONS_LAST_24H, MiscUtils.val(interactionsLast24h));
        if(riskScore!=null) parameters.put(Parameters.RISK_SCORE, MiscUtils.val(riskScore));
        if(complaints!=null) parameters.put(Parameters.COMPLAINTS, MiscUtils.val(complaints));
        if(wasBanned!=null) parameters.put(Parameters.WAS_BANNED, MiscUtils.val(wasBanned));
        if(deliveredProducts!=null) parameters.put(Parameters.DELIVERED_PRODUCTS, MiscUtils.val(deliveredProducts));
        if(canceledPayments!=null) parameters.put(Parameters.CANCELED_PAYMENTS, MiscUtils.val(canceledPayments));
        if(rating!=null) parameters.put(Parameters.RATING, MiscUtils.val(rating));
        if(registrationAge!=null) parameters.put(Parameters.REGISTRATION_AGE, MiscUtils.val(registrationAge));
        if(enable3dSecure!=null) parameters.put(Parameters.ENABLE_3D_SECURE, MiscUtils.val(enable3dSecure));
        return parameters;
    }
