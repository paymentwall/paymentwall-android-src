 ...
 * glide.load.engine.DiskCacheStrategy#RESOURCE}.
 *
 * <p>
...
public final class Option<T> {
   ...
   * glide.load.Option.CacheKeyUpdater} to update disk cache keys.
   *
   * @param key A unique package prefixed {@link String} that identifies this option (must be
   *            stable across builds, so {@link Class#getName()} should <em>not</em> be used).
   */
  ...
  /**
   * Returns a new {@link Option} that uses the given {@link
   * glide.load.Option.CacheKeyUpdater} to update disk cache keys and provides
   ...
   * value using the {@link glide.load.Option.CacheKeyUpdater} optionally provided in
   * the constructor.
   */
