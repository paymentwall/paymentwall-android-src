...
public interface DataRewinder<T> {
   ...
   * {@link glide.load.data.DataRewinder}s.
   *
   * @param <T> The type of data that the {@link glide.load.data.DataRewinder} will
   *            wrap.
   */
  interface Factory<T> {
    /**
     * Returns a new {@link glide.load.data.DataRewinder} wrapping the given data.
     */
    DataRewinder<T> build(T data);

    /**
     * Returns the class of data this factory can produce
     * {@link glide.load.data.DataRewinder}s for.
     */
