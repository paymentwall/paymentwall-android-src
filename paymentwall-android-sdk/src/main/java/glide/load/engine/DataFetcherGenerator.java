...
/**
 * Generates a series of {@link glide.load.data.DataFetcher DataFetchers} using
 * registered {@link glide.load.model.ModelLoader ModelLoaders} and a model.
 */
interface DataFetcherGenerator {
  /**
   * Called when the generator has finished loading data from a
   * {@link glide.load.data.DataFetcher}.
   */
  ...
  /**
   * Attempts to a single new {@link glide.load.data.DataFetcher} and returns true if
   * a {@link glide.load.data.DataFetcher} was started, and false otherwise.
   */
  boolean startNext();
