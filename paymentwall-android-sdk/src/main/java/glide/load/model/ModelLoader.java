 ...
 * {@link glide.load.ResourceDecoder} to decode a resource.
 */
public interface ModelLoader<Model, Data> {

  /**
   * Contains a set of {@link glide.load.Key Keys} identifying the source of the load,
   * alternate cache keys pointing to equivalent data, and a
   * {@link glide.load.data.DataFetcher} that can be used to fetch data not found in
   * cache.
   *
  ...
  /**
   * Returns a {@link glide.load.model.ModelLoader.LoadData} containing a
   * {@link glide.load.data.DataFetcher} required to decode the resource
   * represented by this model, as well as a set of {@link glide.load.Key Keys} that
   * identify the data loaded by the {@link glide.load.data.DataFetcher} as well as an
   ...
   *               {@link glide.request.target.Target#SIZE_ORIGINAL} to indicate that
