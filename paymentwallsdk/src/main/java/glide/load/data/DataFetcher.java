 ...
 *
 * <p> A new instance is
 * created per resource load by {@link glide.load.model.ModelLoader}. {@link
 * #loadData(glide.Priority, glide.load.data.DataFetcher.DataCallback)}
 ...
 * {@link #loadData(glide.Priority,
 * glide.load.data.DataFetcher.DataCallback)}} is called, then so {@link #cleanup()}
 * will be called. </p>
 *
...
public interface DataFetcher<T> {
   ...
   * finally block after the data provided by {@link #loadData(glide.Priority,
   * glide.load.data.DataFetcher.DataCallback)} has been decoded by the
   * {@link glide.load.ResourceDecoder}.
   *
   * <p> Note - this method will be run on a background thread so blocking I/O is safe. </p>
   */
  void cleanup();
  ...
  /**
   * Returns the {@link glide.load.DataSource} this fetcher will return data from.
   */
